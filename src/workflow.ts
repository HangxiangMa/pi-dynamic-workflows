...
// Parse-time author hint. Detection runs on the AST so comments and prose strings
// containing these names do not reject an otherwise deterministic workflow.
...
export function parseWorkflowScript(script: string): { meta: WorkflowMeta; body: string } {
  ...
  const staticName = (node: AnyNode | undefined): string | undefined => {
    if (!node) return undefined;
    if (node.type === "Identifier") return node.name as string;
    if (node.type === "Literal" && typeof node.value === "string") return node.value;
    if (node.type === "TemplateLiteral" && (node.expressions as AnyNode[]).length === 0) {
      const quasi = (node.quasis as AnyNode[])[0];
      return quasi?.value?.cooked as string | undefined;
    }
    if (node.type === "BinaryExpression" && node.operator === "+") {
      const left = staticName(node.left as AnyNode);
      const right = staticName(node.right as AnyNode);
  ...
  const isForbidden = (node: AnyNode): boolean => {
    ...
    if (current.type === "NewExpression") {
      return staticName(current.callee as AnyNode) === "Date" && (current.arguments as AnyNode[]).length === 0;
    }
    if (current.type === "CallExpression" || current.type === "OptionalCallExpression") {
      const callee = unwrap(current.callee as AnyNode);
  ...
  const walk = (node: unknown): void => {
    if (forbidden || !node || typeof node !== "object") return;
    if (isForbidden(node as AnyNode)) {
      forbidden = true;
      return;
    }
